    ...
    return (
        <header className="flex flex-wrap lg:justify-start lg:flex-nowrap w-full h-16 bg-white text-sm py-3 dark:bg-neutral-800">
            <nav className="max-w-[100rem] mx-auto px-0 lg:px-4 flex flex-wrap basis-full items-center justify-between">
                <Link className="lg:order-1 flex-none text-xl font-semibold text-black dark:text-white focus:outline-none focus:opacity-80 px-4" href="#">Techs & The City</Link>
                    ...
                    <div className="flex flex-col gap-5 mt-5 lg:flex-row lg:items-center lg:mt-0 lg:ps-5 px-4 max-lg:pb-4">
                        {
                            navLinks.map((link, index) => (
                                <div key={index}>
                                    <Link href={link.href} className={`font-medium focus:outline-none ${pathName === link.href ? "text-blue-500" : "text-gray-600 hover:text-gray-400 focus:outline-none focus:text-gray-400 dark:text-neutral-400 dark:hover:text-neutral-500 dark:focus:text-neutral-500"}`}>{link.name}</Link>
